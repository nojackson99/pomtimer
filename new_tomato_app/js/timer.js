...
// -------------- SOUND VARIABLES -------------
const alyssaSound = new Audio(
  '../misc_project_files/sounds/alyssa_timer_end.mp3'
);
...
timer.addEventListener('secondsUpdated', () => {
  ...
  // updates timer display with time left on timer
  $('#timer-display').html(currentTime);
});

// Sets behavior when timer finishes
timer.addEventListener('targetAchieved', () => {
  ...
  // if pomodoro session completed switch to appropriate break length
  if (Theme.siteBody.classList.contains(`${Theme.pomTheme}`)) {
    ...
    if (workSessionNumber === longBreakInterval) {
      ...
      Theme.changeTheme(Theme.sessionLengths.longBreak, Theme.longTheme);
      // switch theme to short break
    } else {
      Theme.changeTheme(Theme.sessionLengths.shortBreak, Theme.shortTheme);
    }
    // if break session completed switch theme to pomodoro
  } else {
    Theme.changeTheme(Theme.sessionLengths.pomodoro, Theme.pomTheme);
  }
...
startButton.addEventListener('click', () => {
  startTimer(Theme.sessionLength);
});
